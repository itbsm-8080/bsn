...
begin
        ...
        s:= 'SELECT sup_kode Supplier,sup_nama Nama,sup_alamat Alamat,'
+ ' (SELECT ifnull(SUM(kredit-debet),0) FROM kartu_hutang WHERE supplier=sup_kode AND'
+ ' tanggal < '+QuotD(startdate.DateTime)+') Saldo_awal,'
+ ' (SELECT ifnull(SUM(kredit),0) FROM kartu_hutang WHERE supplier=sup_kode AND'
+ ' tanggal between '+QuotD(startdate.DateTime)+' AND '+QuotD(enddate.DateTime)+') Kredit,'
+ ' (SELECT ifnull(SUM(debet),0) FROM kartu_hutang WHERE supplier=sup_kode AND'
+ ' tanggal between '+QuotD(startdate.DateTime)+' AND '+QuotD(enddate.DateTime)+') Debet,'
+ ' (SELECT ifnull(SUM(kredit-debet),0) FROM kartu_hutang WHERE supplier=sup_kode AND'
+ ' tanggal <= '+QuotD(enddate.DateTime)+') Saldo_akhir,sup_internal Internal '
+ ' FROM tsupplier';
...
//        + ' order by Tanggal';
        ...
        Skolom :='Supplier,Nama,Alamat,Saldo_Awal,Debet,Kredit,Saldo_Akhir,Internal';
        ...
        cxGrid1DBTableView1.Columns[3].Summary.GroupFooterKind:=skSum;
        cxGrid1DBTableView1.Columns[3].Summary.GroupFooterFormat:='###,###,###,###';
...
procedure TfrmListSaldoHutang.LihatFakturPenjualan1Click(
  Sender: TObject);
var
  frmFP: TfrmFP;
